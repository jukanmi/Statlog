...
- [x] Skeleton Screens: Show placeholder UI while waiting for the AI response in the Quiz or Stat screens.
- [x] Loading Tips: Display random study tips or user stats during the waiting period.

## 2. Defensive Coding (Stability & Validation)
- [x] Schema Validation: Implement runtime validation (e.g., using Zod) to ensure the AI's JSON response matches the expected `AIStats` and `AIQuizItem` interfaces.
