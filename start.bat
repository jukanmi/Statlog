...
REM    frontend : vite     http://localhost:8080
REM  Each runs in its own console window. Use stop.bat to stop.
REM ============================================================
cd /d "%~dp0"

echo [1/2] Starting backend (uvicorn :8000)...
start "Statlog Backend" cmd /k "call "%~dp0.venv\Scripts\activate.bat" && python -m uvicorn main:app --host 127.0.0.1 --port 8000"
